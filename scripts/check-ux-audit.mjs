...
const checkoutFlow = fs.readFileSync("js/checkout-flow.js", "utf8");
const adminSchedule = fs.readFileSync("js/admin-schedule.js", "utf8");
...
assert.doesNotMatch(loader, /ux-audit-fixes\.js/, "Las correcciones UX estables ya no deben depender de un sidecar genérico.");
...
assert.match(checkoutFlow, /checkoutPromotionSection/, "Checkout Flow debe integrar directamente la promoción en la revisión.");
assert.match(checkoutFlow, /section\.appendChild\(promotion\)/, "La promoción debe moverse explícitamente dentro de Revisa y crea el pedido.");
assert.match(adminSchedule, /querySelectorAll\("input, textarea, select"\)/, "Admin Schedule debe ser dueño de la normalización de sus controles.");
assert.match(uxCss, /checkout-review-flow-section > \.checkout-promotion-section/, "La promoción debe mantener layout explícito dentro de revisión.");
...
assert.match(indexHtml, /data-kantu-admin-polish="true"/, "Admin polish debe cargarse declarativamente.");
assert.match(indexHtml, /data-kantu-ux-audit="true"/, "UX audit CSS debe cargarse declarativamente.");

console.log("UX responsibilities consolidated; topper customization and admin polish contracts OK");
