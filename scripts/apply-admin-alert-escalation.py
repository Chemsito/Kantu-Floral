...
js_path = Path('js/admin-growth.js')
css_path = Path('css/kantu-growth.css')
...
def replace_once(text, old, new, label):
    if old not in text:
        raise RuntimeError(f'No se encontró bloque: {label}')
...
def sub_once(text, pattern, repl, label):
    ...
    if count != 1:
        raise RuntimeError(f'No se encontró patrón: {label}')
    return new_text

js = replace_once(js, '''    const alertState = {
        rows: [],
        initialized: false,
        knownKeys: new Set(),
        audioReady: false,
...
js = sub_once(js,
r'''    async function loadAdminAlerts\(\{ forceSound = false \} = \{\}\) \{.*?\n    \}\n\n    async function openAdminAlertTarget''',
...
marker = '/* Admin alert escalation v2 */'
if marker not in css:
    css += '''

/* Admin alert escalation v2 */
...
print('Admin alert escalation applied')
