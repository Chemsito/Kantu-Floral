...
begin
    ...
    join (
        select
            oi.product_id,
            sum(oi.quantity)::bigint as required_quantity
        from public.order_items as oi
        where oi.order_id = p_order_id
    ...
    if exists (
        ...
        raise exception using
            errcode = 'P0001',
            message = 'ORDER_PRODUCT_NOT_FOUND';
    end if;
    ...
    if exists (
        ...
        where p.stock is null
           or p.stock < requested.required_quantity
    ) then
        raise exception using
            errcode = 'P0001',
...
revoke all
on function public.confirm_paid_order(bigint, text, timestamptz)
from public;
